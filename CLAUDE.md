...
test/
  ...
  spec/float_config_spec.lua      – Float config: defaults, padding, max_width/height (fractional/absolute), border, win_options, centering, partial merge, property-based bounds/centering, viewer/editor integration
...
- **Configurable float layout**: `setup({ float = { padding, max_width, max_height, border, win_options } })` controls note editor/viewer floating windows. `padding` (int, default 3) is distance from editor edges. `max_width`/`max_height` (0–1 = fraction, >1 = absolute columns/rows, default 0.9). `border` (string or char array, default `"rounded"`). `win_options` (table, default `{ winblend = 0 }`). Floats are centered relative to the editor. `M._compute_float_config(content_w, content_h, title?)` computes the `nvim_open_win` config table. `M._float_opts` stores resolved options.
